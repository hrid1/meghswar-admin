...
import { allAdvanceInvoiceColumns } from "./AllAdvanceInvoiceCol";
import { allAdvanceInvoiceFakeData, type AllAdvanceInvoiceRow } from "./fakeData";

type RowId = string | number;

export default function AllAdvanceInvoiceTable() {
    const [selectedIds, setSelectedIds] = useState<RowId[]>([]);
    const [search, setSearch] = useState("");

    const filteredRows = useMemo(() => {
        const q = search.trim().toLowerCase();
        if (!q) return allAdvanceInvoiceFakeData;

        return allAdvanceInvoiceFakeData.filter((r) => {
            return (
    ...
    const columns = useMemo(() => allAdvanceInvoiceColumns(), []);
    ...
    return (
        <div className="space-y-4 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
            ...
            {/* Table */}
            <div className="mt-6">
                <DataTable<AllAdvanceInvoiceRow>
                    columns={columns}
                    data={filteredRows}
                    selectable
                    minWidth={800}
