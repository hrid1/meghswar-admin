"use client";

import React from "react";
import type { Column } from "@/components/reusable/DataTable";
import type { MerchantListRow } from "./fakeData";
import Link from "next/link";
import { AppButton } from "@/components/reusable/CustomButton";

function money(n: number) {
  return new Intl.NumberFormat("en-US").format(n);
}

function StatusPill({ status }: { status: MerchantListRow["status"] }) {
  const cls =
    status === "Active"
      ? "bg-green-100 text-green-700"
      : "bg-gray-200 text-gray-600";
  return (
    <span className={`inline-flex rounded-full px-3 py-1 text-[11px] font-semibold ${cls}`}>
      {status}
    </span>
  );
}

export const merchantListColumns = (): Column<MerchantListRow>[] => [
  {
    key: "merchant",
    header: "Merchant",
    width: "18%",
    cellClassName: "align-middle",
    render: (r) => (
      <div className="flex items-center gap-3">
        <div className="h-10 w-10 rounded-full bg-gray-200 overflow-hidden shrink-0">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={r.merchant.avatarUrl || "https://i.pravatar.cc/80?img=12"}
            alt={r.merchant.name}
            className="h-full w-full object-cover"
          />
        </div>
        <div className="flex flex-col leading-4">
          <span className="text-sm font-semibold text-gray-900">{r.merchant.name}</span>
          <span className="text-xs text-gray-500">{r.merchant.phone}</span>
        </div>
      </div>
    ),
  },
  {
    key: "totalParcel",
    header: "Total Parcel",
    width: "9%",
    cellClassName: "align-middle text-center",
    headerClassName: "text-center",
    render: (r) => <span className="text-sm font-semibold">{r.totalParcel}</span>,
  },
  {
    key: "parcelDelivered",
    header: "Parcel Delivered",
    width: "10%",
    cellClassName: "align-middle text-center",
    headerClassName: "text-center",
    render: (r) => <span className="text-sm font-semibold">{r.parcelDelivered}</span>,
  },
  {
    key: "parcelReturned",
    header: "Parcel Returned",
    width: "10%",
    cellClassName: "align-middle text-center",
    headerClassName: "text-center",
    render: (r) => <span className="text-sm font-semibold">{r.parcelReturned}</span>,
  },
  {
    key: "collectedAmount",
    header: "Collected Amount",
    width: "10%",
    cellClassName: "align-middle text-right",
    headerClassName: "text-right",
    render: (r) => (
      <span className="text-sm font-semibold text-green-600">
        {"\u09F3"} {money(r.collectedAmount)}
      </span>
    ),
  },
  {
    key: "deliveryCharge",
    header: "Delivery Charge",
    width: "10%",
    cellClassName: "align-middle text-right",
    headerClassName: "text-right",
    render: (r) => (
      <span className="text-sm font-semibold text-green-600">
        {"\u09F3"} {money(r.deliveryCharge)}
      </span>
    ),
  },
  {
    key: "totalPaid",
    header: "Total Paid",
    width: "12%",
    wrap: true,
    cellClassName: "align-middle text-center",
    headerClassName: "text-center",
    render: (r) => (
      <div className="flex flex-col items-center gap-0.5">
        <span className="text-sm font-semibold text-green-600">
          {"\u09F3"} {money(r.totalPaid.amount)}
        </span>
        <span className="text-[10px] text-gray-500 leading-4 text-center">
          Last Paid {r.totalPaid.lastPaidAt}
        </span>
      </div>
    ),
  },
  {
    key: "status",
    header: "Status",
    width: "9%",
    cellClassName: "align-middle text-center",
    headerClassName: "text-center",
    render: (r) => <StatusPill status={r.status} />,
  },
  {
    key: "advancePayment",
    header: "Advance Payment",
    width: "12%",
    cellClassName: "align-middle text-center",
    headerClassName: "text-center",
    render: (r) => (
      <span
        className={`text-sm font-semibold ${
          r.advancePayment === "ON" ? "text-green-700" : "text-gray-900"
        }`}
      >
        {r.advancePayment}
      </span>
    ),
  },
  {
    key: "view",
    header: "View",
    width: "8%",
    cellClassName: "align-middle text-center",
    headerClassName: "text-center",
    render: (r) => (
      <div onClick={(e) => e.stopPropagation()}>
        <Link href={`./merchant-list/${encodeURIComponent(r.id)}`}>
          <AppButton variantType="outline" className="px-4 py-2 text-xs rounded-md">
            View
          </AppButton>
        </Link>
      </div>
    ),
  },
];
