...
export type ReturnToMerchantRow = {
  parcelid: string;
  reason: string;
  destination: {
    title: string;
...
export const returnToMerchantColumns = (
  _onClickUpdate: (row: ReturnToMerchantRow) => void
): Column<ReturnToMerchantRow>[] => [
  {
    key: "parcelid",
    ...
    render: (row) => {
      ...
      return (
        <div className="flex flex-col gap-0.5">
          <div className="text-sm font-bold text-[#107E3E]">৳ {money}</div>
          <div className="text-[10px] text-gray-500 space-y-0.5">
            <div>Delivery Charge: ৳ 125</div>
            <div>COD Charge: ৳ 12</div>
            <div>Weight Charge: ৳ 50</div>
            <div className="text-[#FE5000]">Discount: ৳ 0</div>
          </div>
        </div>
      );
    },
